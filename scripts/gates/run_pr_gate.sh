...
print_help() {
    cat <<'EOF'
Usage: run_pr_gate.sh --run-id <run_id> [--artifact-root <path>] [--config-profile <profile>]

Run the PR gate skeleton for the L0-bus workspace.

Options:
  --run-id <run_id>            Fixed run identifier.
  --artifact-root <path>       Artifact root. Defaults to artifacts/test/<run_id>.
  --config-profile <profile>   Config profile. Defaults to ci-test.
...
printf 'PR gate skeleton\n'
printf 'run_id=%s\n' "${run_id}"
printf 'artifact_root=%s\n' "${artifact_root}"
printf 'config_profile=%s\n' "${config_profile}"
printf 'suites=bus-unit,bus-service,bus-contract,bus-config,bus-redaction-smoke,bus-integration-fast\n'
